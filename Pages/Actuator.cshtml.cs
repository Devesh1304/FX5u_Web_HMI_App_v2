...
{
    public class ActuatorModel : PageModel
    {
        ...
        {
            _slmpService.SetHeartbeatValue(19); // Set value for Home Screen
        ...
        private readonly Dictionary<string, string> _int32WriteAddressMap = new()
    {
        ...
        public ActuatorModel(ILogger<IndexModel> logger, ISLMPService slmpService)
        {
            _logger = logger;
        ...
        private async Task UpdateModelValuesAsync()
        {
            ...
            {      
                ...
                D2 = (await _slmpService.ReadInt16Async("D2")).Content;
           

                var y02Result = await _slmpService.ReadBoolAsync("Y02", 1);
                ...
                var y03Result = await _slmpService.ReadBoolAsync("Y03", 1);
