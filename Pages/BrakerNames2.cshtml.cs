...
namespace FX5u_Web_HMI_App.Pages
{
    public class BrakerNames2Model : PageModel
    {
        ...
        {
            _slmpService.SetHeartbeatValue(21);
        ...
        public async Task<JsonResult> OnPostSetGujarati([FromBody] SetGujaratiRequest req)
        {
            try
            {
                ...
                // 🔁 Auto-maintain NameTranslations: read English for this index from PLC (D4200 block)
        ...
        private async Task UpdateModelValuesAsync()
        {
            try
            {
                ...
                if (!read.IsSuccess)
                    throw new Exception("Failed to read breaker names (D4200) from PLC: " + read.Message);
